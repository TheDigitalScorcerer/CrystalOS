...
int handle_scrolling(int cursor_offset) {
    ...
    for (int i = 1; i < MAX_ROWS; i++) {
        memcpy((char*)VIDEO_ADDRESS + get_screen_offset(0, i), (char*)VIDEO_ADDRESS + get_screen_offset(0, i - 1), MAX_COLS * 2);
    }
    /*  Blank  the  last  line by  setting  all  bytes  to 0 */
    char* last_line = ((char*)VIDEO_ADDRESS + get_screen_offset(0, MAX_ROWS -1) + VIDEO_ADDRESS);
